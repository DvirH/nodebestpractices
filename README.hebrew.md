...
1. [מבנה פרוייקט מומלץ (5)](#1-project-structure-practices)
2. [טיפול בשגיאות (12) ](#2-error-handling-practices)
3. [סגנון קוד (12) ](#3-code-style-practices)
4. [בדיקות כלליות ואיכות הקוד (13) ](#4-testing-and-overall-quality-practices)
5. [מעבר לפרודקשן (19) ](#5-going-to-production-practices)
6. [שיטות אבטחה (25)](#6-security-best-practices)
7. [ביצועים (2) (בעבודה ✍️)](#7-draft-performance-best-practices)
8. [שיטות עבודה עם דוקר (15)](#8-docker-best-practices)

<br/><br/>

# `1. מבנה פרוייקט מומלץ`

## ![✔] 1.1 בנה את המוצר שלך בעזרת קומפוננטות

**אמ;לק:** הדבר הקשה ביותר לפרוייקטים גדולים הוא תחזוקה של בסיסי קוד עצומים עם אלפי תלויות - בדומה למונוליט אשר מאט את קצב עבודת המתכנים כאשר הם מנסים לפתח פיצ'רים חדשים. במקום זה, חלק את הקוד שלך לחלקים קטנים, כלומר קומפוננטות. כל קומפוננטה תכיל את הקוד הרלבנטי אליה בלבד. כך נבטיח שכל יחידת תכיל כמות קוד קונה ופשוטה הניתנת לתחזוקה.
לחץ 'קרא עוד' מתחת על מנת לצפות בודגמאות של שימוש נכון בדוגמה

**אחרת:** כשמתכנתים המייצרים פיצ'רים חדשים נאבקים על מנת להבין את ההשפעה של השינוי שלהם ומפחדים לשבור תלות של יחידה אחרת הקשורה בה - הפיתוח נעשה איטי במיוחד ומסוכן יותר. זה גם קשה להרחיב את הקף הפרוייקט כאשר יחידות לא מופרדות.

🔗 [**קרא עוד: מבנה בעזרת קומפוננטות**](./sections/projectstructre/breakintcomponents.md)

<br/><br/>

## ![✔] 1.2 תרבד את הקומפוננטות, שמור על כל מרכיב בתחום הגבולות שלו

**TL;DR:** כל קומופוננטה צריכה להכיל 'רבדים' - אובייקט יעודי עבור האתר, הלוגיקה, וגישה לבסיס הנתונים.
זה לא רק שומר על קוד נקי ומופרד אלא מקל על תהליך הבדיקות שיבוצעו. אף על פי שמדובר בתבנית נפוצה, מתכנתים רבים נוטים לערבב בין שכבות כאשר הם מעבירים את אובייקט הבקשה אל מחלקות הניהול לדוגמא:
Express req. דבר זה גורם לאפליקציה להיות תלויה ב frameworks ספציפיים

**אחרת:** אפליקציה אשר מערבבת קוד הקשור לאתר עם רבדים אחרים לא ניתנת לבדיקה, לא לשימוש ע"י משימות מתוזמנות, טריגרים וכו'

🔗 [**קרא עוד: תרבד את האפליקציה שלך**](./sections/projectstructre/createlayers.md)

<br/><br/>

## ![✔] 1.3 תעטוף קוד נפוץ כחבילה של npm
